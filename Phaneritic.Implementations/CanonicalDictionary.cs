    ...
    where TKey : struct, IEquatable<TKey>
{
    ...
    public TValue GetOrAdd(in TKey key, Func<TKey, TValue> addValueFactory)
        => _Dictionary.GetOrAdd(key, addValueFactory);

    public TValue? TryGetValue(in TKey key)
        => _Dictionary.TryGetValue(key, out TValue? _value) ? _value : default;

    public TValue AddOrUpdate(in TKey key, TValue addValue, Func<TKey, TValue, TValue> updateValueFactory)
